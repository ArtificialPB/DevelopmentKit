...
import com.artificial.cachereader.wrappers.rt6.loaders.QuestDefinitionLoader;
...
public class QuestDefinition extends ProtocolWrapper {
    ...
    public QuestDefinition(final QuestDefinitionLoader loader, final int id) {
        super(loader, id);
    }

    @Override
    protected void decodeOpcode(final Stream data, final int opcode) {
        switch (opcode) {
            case 1:
    ...
    public enum QuestClassification {
        ...
        QuestClassification(final int id) {
            this.id = id;
        }
    }

    static {
        final QuestClassification[] values = QuestClassification.values();
        CLASSIFICATION_MAP = new HashMap<Integer, QuestClassification>(values.length);
        for (final QuestClassification value : values) {
